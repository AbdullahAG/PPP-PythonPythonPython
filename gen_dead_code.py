...
def random_oper(t,f,n):
    ...
    for i in range(n):
        if i%6==0:
            f.write(t+"for "+"".join(random.choice(string.ascii_letters) for i in range(random.randint(1,5)))+" in range("+str(random.randint(1,150))+"):\n"+t+"\tprint(m+"+str(random.randint(1,150))+")\n")
        elif i%2==0:
            f.write(t+"print('"+gen_random_str(10)+"')\n")
        elif i%2==1:
...
def random_fun(t, n,f = open("code_dead.txt", "w")):
    ...
    for i in range(n):
        ...
        while(True):
            random_oper(t+"\t",f,random.randint(1,6))
            if random.randint(0,1):
                break          
    if keep_op:          
        f.close()        
